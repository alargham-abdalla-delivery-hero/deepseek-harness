...
网页聊天 Client 中 `render_ui` 的 keyed `tool.call.toolview`：把已结算结果中持久化的 [OpenUI](https://github.com/thesysdev/openui) element tree，在聊天轮次里渲染为实时 React UI。

## 功能说明

在 `tool.call.toolview` slot（[`@deepseek-ai/dsh-client-ui-tool`](../ui-tool/README.zh.md)）上以 `key: 'render_ui'` 注册 `RenderUiView`，采用与 `webToolview` 注册 `WebRow` 相同的模式。未被认领的 key 会回退到通用卡片；安装本插件后，`render_ui` 专属的回退行为会被替换。
