---
description: "Model-facing render_ui tool: parse and validate OpenUI Lang against the shared curated component vocabulary, teach the model the grammar, and self-correct a model that writes the DSL as prose instead of calling the tool."
...
This package registers the model-facing `render_ui` tool: parse and validate [OpenUI Lang](https://github.com/thesysdev/openui) against the shared curated component vocabulary ([`@deepseek-ai/dsh-openui-lang`](../openui-lang/README.md)), teach the model the grammar via a system-prompt section, and steer a model that writes the DSL directly as chat text back onto the tool-call path. A syntax mistake, an unknown component, or a missing required prop is never a thrown error — it is the model's own recoverable output, reported back so the model can correct it on the next call.
...
Mount this plugin to register the `render_ui` tool. It calls `dsh-openui-lang`'s `parseSource()` and returns `{ root, errors, incomplete }` — the shared package's own lenient parse outcome. Only a genuine parser implementation failure throws.
...
|---|---|---|
| `maxCorrectionAttempts` | `2` | Maximum consecutive corrective steers issued for a model that keeps writing OpenUI Lang as chat text instead of calling `render_ui`, before the turn is allowed to close with its text answer unrendered. |
...
Registers one tool, `render_ui(source: string)`, on `ctx.tools`, and one system-prompt section on `ctx.systemPrompt`. `execute` parses `source` with `dsh-openui-lang`'s `parseSource()` and returns `{ root, errors, incomplete }`.
...
Corrections are capped per `Config.maxCorrectionAttempts` so a model that keeps not calling the tool cannot be steered indefinitely; once the cap is reached, the turn closes with its text answer unrendered.
