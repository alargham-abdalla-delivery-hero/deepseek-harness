...
共享的 [OpenUI Lang](https://github.com/thesysdev/openui) 组件词汇（vocabulary）：一份经过整理的 `Library` 定义，同时被 [`@deepseek-ai/dsh-tool-openui`](../tool-openui/README.zh.md)（服务端校验与系统提示词生成）与 [`@deepseek-ai/dsh-client-ui-openui`](../../client/ui-openui/README.zh.md)（网页聊天渲染器）消费。
