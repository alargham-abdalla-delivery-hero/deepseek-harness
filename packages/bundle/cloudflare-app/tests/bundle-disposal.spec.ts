...
import CredentialsD1 from '@deepseek-ai/dsh-credentials-d1'
...
describe('dsh-cloudflare-app bundle removal', () => {
  it('mounts storage-d1, session-persistence-d1, credentials-d1, and the startup guard as one unit, and disposing all four leaves no residual registration behind', async () => {
    ...
    const credentialsFiber = await ctx.plugin(CredentialsD1, D1_CONFIG)
    const startupFiber = await ctx.plugin(CloudflareAppStartup)
    await Promise.all([
      storageFiber.await(), sessionFiber.await(), credentialsFiber.await(), startupFiber.await(),
    ])
    ...
    // Removing the bundle disposes all four plugins it mounts, together.
    await Promise.all([
      storageFiber.dispose(), sessionFiber.dispose(), credentialsFiber.dispose(), startupFiber.dispose(),
    ])
