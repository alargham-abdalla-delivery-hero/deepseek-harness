...
运行 `dsh --profile cloudflare`，你将得到与本地 `dsh --profile web` 完全相同的浏览器 UI、Workspace 管理与聊天体验，只是绑定到 Cloudflare D1 而非本地文件系统。本组合包对 `dsh-web-app` 打 patch（profile 分层顺序：`dsh-base` → `dsh-web-app` → `dsh-cloudflare-app`），而不是重新复制一份：它禁用本地的 `storage-json`/`session-persistence-jsonl` 后端，将 `ctx.storageDomain` 路由到 `@deepseek-ai/dsh-storage-d1`，挂载 `@deepseek-ai/dsh-session-persistence-d1`，并关闭本地浏览器唤起——除此之外的每一行配置（聊天 UI、workspace/会话/设置控制器、目录选择器）都保持不变。当需要把 `dsh` Host 打包进 Cloudflare Container 的 Docker 镜像时选择该 profile；普通本地使用请选择 `web`。本组合包不贡献任何自己的提示词、工具或模式——每一个面向模型的表层都属于它所 patch 的那些配置行，而非本包。
...
这些参数都是 `dsh-web-app` 自己的——本组合包不新增任何参数。`--no-open` 是容器入口所需要的值（容器内没有本地浏览器可供唤起）。使用 `--host 127.0.0.1` 而非 `0.0.0.0`：`dsh-web-app` 自身的命令行会刻意拒绝 `0.0.0.0`（"会将远程代码执行暴露给网络"），而 `127.0.0.1` 对 Cloudflare Container 而言恰好也是架构上正确的取值——该平台通过与容器共享网络命名空间（pod 内部）的代理来访问容器，而非原始的 Docker 桥接网络。仓库根目录下的 `/Dockerfile.cloudflare` 把这条确切的调用方式固化进了它的 `CMD`。
...
缺少其中任何一项都会让启动立即、明确地失败（`cloudflare-app-startup`），甚至在 D1 后端尝试任何 REST 调用之前——见"可观察行为"。
...
| 文件 | 角色 |
|---|---|
| [`cordis.patch.yml`](cordis.patch.yml) | 全部 Cloudflare 相关 patch：禁用本地后端、将 `storage-domain` 路由到 `d1`、插入 `storage-d1`/`session-persistence-d1`/启动检查、禁用本地浏览器唤起 |
...
- [`dsh-storage-d1` README](../../storage/storage-d1/README.zh.md) 与 [`dsh-session-persistence-d1` README](../../session/session-persistence-d1/README.zh.md)——本组合包挂载的两个后端。
...
- **容器镜像打包尚未面向生产环境做精简** —— 镜像执行的是一次完整的 `pnpm install --frozen-lockfile`（每个工作区成员，开发依赖与运行时依赖一并安装——本仓库"peerDependency 通过 devDependency 满足"的约定意味着仅生产环境的安装会悄悄丢弃若干运行时导入实际需要的包），而非仅生产环境的精简安装（例如通过 `pnpm deploy`）；镜像可以正常工作，但远大于必要体积。精简工作被推迟，与真实的 Cloudflare 部署工作一并跟踪。
...
- **`$DSH_HOME` 位于容器自身的临时磁盘上** —— `/Dockerfile.cloudflare` 设置了 `DSH_HOME=/app/.dsh-home`，让 profile 解析的簿记有一个可写位置，但其下的任何内容都不会在容器重启后持久保留；只有经由 `dsh-storage-d1`/`dsh-session-persistence-d1` 存入 D1 的状态才会存活。
- **在本仓库自身的开发环境中，容器实际启动的验证受限于沙箱内存** —— 在一个约 1.9 GiB 内存的 Docker 守护进程中，`docker build` 至少完整成功过一次；但在该内存上限下，镜像内约 1000 个包的全新 `pnpm install` 无论如何调整并发度都会偶发被 OOM 杀死。这是该特定沙箱的限制，而非镜像本身的问题。在拥有更多构建内存的环境中，或通过真实的 Cloudflare Container 部署重新验证，之后再将"容器可实际启动"视为已验证。本仓库 CI 中尚无自动化的容器启动测试；与 `packages/cloudflare/cloudflare-worker`（真正把该镜像作为 Cloudflare Container 运行的 Worker + Durable Object）一并推迟。
